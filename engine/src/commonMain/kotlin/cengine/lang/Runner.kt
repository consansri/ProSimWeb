...
abstract class Runner<T: LanguageService>(val name: String) {
    ...
    suspend fun run(ioContext: ConsoleContext, project: Project, file: VirtualFile, vararg attrs: String): Boolean {
        return ioContext.runWithContext(project, DEFAULT_FILEPATH_ATTR, file.path.relativeTo(ioContext.directory).toString(), *attrs)
    }
