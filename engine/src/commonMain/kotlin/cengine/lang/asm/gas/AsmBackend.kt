...
class AsmBackend<T : AsmCodeGenerator.Section>(
    ...
    val integerEvaluator = IntegerExpressionEvaluator<AsmEvaluationContext> { name, element, context ->
    ...
    suspend fun assemble(): ByteArray? {
        io.info("Starting assembly for ${entryPsiFile.file.path}")
        try {
            ...
            io.info("Assembly finished, generating output file.")
    ...
    private suspend fun runLinking(): Boolean {
        ...
        try {
            ...
            io.info("Phase 1: Linking complete.")
    ...
    private fun runSymbolAndSectionPass(): Boolean {
        io.info("Phase 3: Symbol Definitions & Section Allocation Pass...")
        var success = true
        ...
        io.info("Phase 3: Symbol/Section Pass complete (${if (success) "OK" else "with errors"}).")
        return success
    }
    ...
    private fun runAddressResolution(): Boolean {
        io.info("Phase 4: Resolving Addresses...")

        try {
            codeGenerator.orderSectionsAndResolveAddresses() // Abstract method call

            // Debug: Print section addresses
            io.info("Section Addresses:")
            codeGenerator.sections.forEach { sec ->
                io.info("  ${sec.name}: Addr=0x${sec.address.toString(16)}, Size=${sec.content.size}")
            }
            // Debug: Print symbol addresses/values
            io.info("Symbol Table:")
            codeGenerator.symbols.forEach { sym ->
                when (sym) {
                    is AsmCodeGenerator.Symbol.Abs -> io.info("  Abs: ${sym.name} = ${sym.value} (${sym.binding}) in ${sym.section.name}")
                    is AsmCodeGenerator.Symbol.Label -> io.info("  Label: ${sym.name} @ 0x${sym.address().toString(16)} (${sym.binding}) in ${sym.section.name} (offset ${sym.offset})")
                }
            }

            io.info("Phase 4: Address Resolution complete.")
            return true
        } catch (e: Exception) {
    ...
    private fun runCodeGenerationPass(): Boolean {
        io.info("Phase 5: Code Generation Pass...")
        var success = true
        ...
        codeGenerator.sections.forEach { section ->
            ...
            if (section.type == Shdr.SHT_NOBITS) {
                io.info("Skipping code generation for NOBITS section ${section.name}.")
                section.reservations.clear() // Clear reservations as they are irrelevant
                return@forEach
            }

            io.info("Generating code for section ${section.name}...")
        ...
        io.info("Phase 5: Code Generation complete (${if (success) "OK" else "with errors"}).")
        return success
    }
            ...
            AsmDirective.AssemblyControl.AssemblyControlT.SET,
                ...
                return try {
                    ...
                    if (existing != null && type != AsmDirective.AssemblyControl.AssemblyControlT.SET) {
                        ...
                        codeGenerator.getOrCreateAbsSymbolInCurrentSection(symName, value)
                        io.info("Preprocessed '$type': $symName = $value")
                        true
                    }
                } catch (e: Exception) {
    ...
    private fun handleSectionControlDirective(directive: AsmDirective.SectionControl): Boolean {
        ...
        try {
            ...
            io.info("Switched to section: ${codeGenerator.currentSection.name}")
            return true
        } catch (e: Exception) {
            ...
            AsmDirective.SymbolManagement.SymbolManagementT.WEAK,
                ...
                directive.symbols.forEach { symbolToken ->
                    ...
                    if (symbol == null) {
                        ...
                        io.info("Set binding for symbol '$name' to $binding")
                    }
                }
            }
        ...
        try {
            ...
            when (type) {
                AsmDirective.Alignment.AlignmentT.ALIGN, AsmDirective.Alignment.AlignmentT.P2ALIGN -> {
                    ...
                    if (misalignment != BigInt.ZERO) {
                        ...
                        if (maxSkip != null && paddingNeeded > maxSkip) {
                            io.info("Skipping alignment for '.$type': padding needed ($paddingNeeded) exceeds max skip ($maxSkip)")
                        } else {
                            io.info("Aligning section '${section.name}' from offset $currentOffsetBigInt by $paddingNeeded bytes (boundary $boundary) using fill $fillValue")
                ...
                AsmDirective.Alignment.AlignmentT.FILL -> {
                    ...
                    io.info("Filling $repeatCount times with $itemSize-byte pattern ${value.toString(16)}")
                ...
                AsmDirective.Alignment.AlignmentT.SPACE,
                    ...
                    if (sizeToSkip > 0) {
                        io.info("Skipping $sizeToSkip bytes using fill $fillValue in section '${section.name}' using '.$type'")
                        try {
                ...
                AsmDirective.Alignment.AlignmentT.ZERO -> {
                    ...
                    if (sizeToZero > 0) {
                        io.info("Writing $sizeToZero zero bytes in section '${section.name}' using '.zero'")
                        try {
    ...
    private fun handleSpaceOrZeroInNobits(directive: AsmDirective.Alignment, type: AsmDirective.Alignment.AlignmentT, section: T): Boolean {
        ...
        try {
            ...
            if (size > 0) {
                io.info("Reserving $size bytes in NOBITS section '${section.name}' using '.$type'")
